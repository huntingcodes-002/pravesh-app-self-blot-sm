...
export default function Step2Page() {
  ...
  // Autosave form data on change
  useEffect(() => {
    if (currentLead) {
        updateLead(currentLead.id, {
            formData: { ...currentLead.formData, step2: {...formData, isMobileVerified} },
  ...
  const handleSendOtp = () => {
    ...
    setTimeout(() => {
      ...
      setIsOtpSent(true);
      toast({ title: 'OTP Sent', description: `Mock OTP (${MOCK_OTP}) sent successfully.`, className: 'bg-blue-50 border-blue-200' });
    }, 1500);
  };
  ...
  const handleVerifyOtp = () => {
    if (otp === MOCK_OTP) {
      ...
      setIsOtpSent(false);
      setOtp('');
      toast({ title: 'Verification Successful', description: 'Mobile number verified.', className: 'bg-green-50 border-green-200' });
    } else {
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <RadioGroup value={formData.salutation} onValueChange={(value) => setFormData({ ...formData, salutation: value })} className="flex space-x-4 mt-2">
                <div className="flex items-center space-x-2"><RadioGroupItem value="Mr" id="mr" /><Label htmlFor="mr">Mr</Label></div>
                <div className="flex items-center space-x-2"><RadioGroupItem value="Mrs" id="mrs" /><Label htmlFor="mrs">Mrs</Label></div>
                <div className="flex items-center space-x-2"><RadioGroupItem value="Ms" id="ms" /><Label htmlFor="ms">Ms</Label></div>
              </RadioGroup>
            </div>
            ...
            <div>
              ...
              <div className="flex space-x-2 items-center">
                <div className="relative flex-1">
                  ...
                  <Input id="mobile" type="tel" maxLength={10} value={formData.mobile} onChange={(e) => setFormData({ ...formData, mobile: e.target.value.replace(/\D/g, '') })} placeholder="Enter 10-digit mobile number" className="h-12 pl-[60px] pr-12" disabled={isMobileVerified || isOtpSent} />
                  {isMobileVerified && <CheckCircle className="w-5 h-5 text-green-600 absolute right-3 top-1/2 transform -translate-y-1/2" />}
                </div>
                <Button onClick={handleSendOtp} disabled={isMobileVerified || isSendingOtp || isOtpSent || formData.mobile.length !== 10} className="h-12 w-32 bg-blue-500 hover:bg-blue-600">
            ...
            {isOtpSent && !isMobileVerified && (
              <div className="space-y-3 p-4 border border-blue-200 rounded-lg bg-blue-50">
                ...
                <Button onClick={handleVerifyOtp} disabled={otp.length !== 6} className="w-full h-10 bg-green-600 hover:bg-green-700 font-semibold">Verify OTP</Button>
              </div>
            )}
          </div>
